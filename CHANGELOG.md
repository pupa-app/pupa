...
- **Tapping a notification while the app is fully closed now opens the chat and
  ...
  `runAgent`/prefill. It now installs at the UIKit launch hook, before that tap
  arrives. Taps while the app was already open were unaffected.
...
- **Your apps keep their memories through this update.** The folder each app
  stores its memories in was renamed in `0.0.249` to something that can't drift
  when you rename the app — but nothing moved what was already there, so an
  updated app would have opened to a blank slate with its notes, skills and
  subagents orphaned beside it. First launch now moves them across. Nothing to
  do, and nothing to export beforehand.
...
- **Renaming an app no longer risks losing its memories.** Memory folders were
  ...
  Existing memories move themselves across on first launch — see `0.0.252`.
  (Before that fix this was a breaking change requiring an export/import
  round-trip; `0.0.249`–`0.0.251` were never released, so no install is
  affected.)
...
- **Restoring an app brings its subagents and skills back.** After iCloud
  removed an app's data, restoring it left empty `pupa/agents/` and
  `pupa/skills/` folders — the agent roster read empty and custom skills didn't
  load. The bodies were never destroyed (the mirror quarantines every file it
  removes), but nothing brought them back. Restoring now re-materializes the
