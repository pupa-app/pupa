...
struct NavStateTests {
    ...
    private var tabsOfA: [SidebarSelection] {
        [.miniAppHome(appA), .miniAppAgents(appA), .miniAppMemories(appA)]
    }

    @Test("the launch page is recorded, so the first navigation keeps it")
    func launchPageSurvivesFirstNavigation() {
        var nav = NavState(rootPage: .miniAppHome(appA))
        nav.setRoot(.miniAppAgents(appA))

        #expect(nav.mounted.contains(.miniAppHome(appA)), "launch page was torn down")
        #expect(nav.panes(from: tabsOfA) == [.miniAppHome(appA), .miniAppAgents(appA)])
    }
    ...
    func subjectIsDerived() {
        ...
        #expect(NavState(rootPage: .miniAppHome(appA)).subject == .miniApp(appA))
    ...
    func unvisitedTabsAreNotMounted() {
        let nav = NavState(rootPage: .miniAppHome(appA))
        // Home is the root; agents and memories have never been opened.
        #expect(nav.panes(from: tabsOfA) == [.miniAppHome(appA)])
    }
    ...
    func rootIsAlwaysMounted() {
        var nav = NavState(rootPage: .miniAppHome(appA))
        nav.setRoot(.miniAppMemories(appA))
        #expect(nav.panes(from: tabsOfA).contains(.miniAppMemories(appA)))
    }
    ...
    func subjectChangeResets() {
        var nav = NavState(rootPage: .miniAppHome(appA))
        nav.setRoot(.miniAppAgents(appA))
        nav.setRoot(.miniAppHome(appB))

        #expect(nav.mounted == [.miniAppHome(appB)])
        #expect(nav.subject == .miniApp(appB))
        #expect(!nav.mounted.contains(.miniAppAgents(appA)))
    }
