...
/// miniApp, or a concurrently running turn.
@MainActor
...
struct ToolGateIsolationTests {
    ...
    /// Two miniApps of the tracker type, each carrying a tracker *and* a
    /// calendar component so both kind gates are advertised in both.
    private func makeStore(count: Int = 2) -> (store: MiniAppStore, ids: [UUID]) {
        MiniAppTypeRegistry.shared.registerBuiltins()
        let miniApps = (0..<count).map { i in
        ...
        for miniApp in miniApps {
            store.addComponent(kind: "tracker", name: "T", iconSystemName: "book", miniAppId: miniApp.id)
            store.addComponent(kind: "calendar", name: "C", iconSystemName: "calendar", miniAppId: miniApp.id)
        }
    ...
    func gateHandlerWritesOnlyItsOwnState() async throws {
        ...
        AppTools.registerToolGates(on: registryA, miniAppType: type, toolGateState: gateA)
        AppTools.registerToolGates(on: registryB, miniAppType: type, toolGateState: gateB)
    ...
    func statesAreIndependent() {
        ...
        let allowedA = ChatViewModel.allowedToolNames(scope: .miniApp(ids[0]), store: store, toolGateState: a)
        let allowedB = ChatViewModel.allowedToolNames(scope: .miniApp(ids[0]), store: store, toolGateState: b)
    ...
    /// Same miniApp, two conversations. The coordinator keys sessions on
    /// `(scope, threadId)`, so thread 2 must still see the gate after thread 1
    /// unlocked the tracker tools.
    @Test("Two threads in the same miniApp scope have independent gates")
    func gatesAreIsolatedPerThread() async throws {
        let (store, ids) = makeStore(count: 1)
        let coord = makeCoordinator(store: store)
        let t1 = coord.session(for: .miniApp(ids[0]), threadId: "thread-1")
        let t2 = coord.session(for: .miniApp(ids[0]), threadId: "thread-2")
        #expect(t1 !== t2)
    ...
    func newThreadStartsClosed() async throws {
        let (store, ids) = makeStore(count: 1)
        let coord = makeCoordinator(store: store)
        let first = coord.session(for: .miniApp(ids[0]), threadId: store.currentThreadId(for: .miniApp(ids[0])))
        try await callGate("get_tools_memories", on: first)
        #expect(first.currentAllowedToolNames.contains("lsMemories"))

        first.newThread()
        let next = coord.session(for: .miniApp(ids[0]))
    ...
    @Test("A gate opened in one miniApp stays shut in a sibling miniApp")
    func gatesAreIsolatedPerMiniApp() async throws {
    ...
    /// Many interleaved activations across many threads of the same miniApp:
    ...
    func interleavedActivationsStayPut() async throws {
        ...
        let sessions = kinds.indices.map { coord.session(for: .miniApp(ids[0]), threadId: "t\($0)") }
