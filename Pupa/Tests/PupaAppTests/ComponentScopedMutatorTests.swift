...
/// component. The kind-routed fallback on `MiniAppStore.mutate` no longer
...
struct ComponentScopedMutatorTests {
    ...
    /// Builds a MiniApp with two tracker components, each holding a single
    ...
    private func freshFixture() -> Fixture {
        ...
        let rowAInA = store.addItem(["title": "in-A"], miniAppId: miniApp.id, componentId: trackerA)!
        ...
        let rowAInB = store.addItem(["title": "in-B"], miniAppId: miniApp.id, componentId: trackerB)!
    ...
    func patchScopedToInactiveComponent() {
        ...
        #expect(ok)
        #expect(trackerItem(f.store, miniAppId: f.miniAppId, componentId: f.trackerA, itemId: f.rowAInA)?.values["title"] == "patched")
        // Tracker B's row untouched.
        #expect(trackerItem(f.store, miniAppId: f.miniAppId, componentId: f.trackerB, itemId: f.rowAInB)?.values["title"] == "in-B")
    }
    ...
    func patchWithoutComponentIdIgnoresActive() {
        ...
        f.store.setActiveComponent(componentId: f.trackerB, miniAppId: f.miniAppId)
    ...
    /// `componentId`, so on a multi-tracker miniApp they silently routed to
    ...
    func addItemScopedToViewedTracker() {
        ...
        let newId = f.store.addItem(["title": "diary row"], miniAppId: f.miniAppId, componentId: f.trackerB)
        #expect(newId != nil)
        // Landed in B…
        #expect(trackerItem(f.store, miniAppId: f.miniAppId, componentId: f.trackerB, itemId: newId!)?.values["title"] == "diary row")
        // …and NOT in A (which still holds only its seed row).
        guard let appA = f.store.miniApps.first(where: { $0.id == f.miniAppId }),
    ...
    func setFilterScopedToViewedTracker() {
        let f = freshFixture()
        f.store.setFilter(field: "title", value: "x", miniAppId: f.miniAppId, componentId: f.trackerB)
        func filterOf(_ id: String) -> [String: String]? {
            guard let app = f.store.miniApps.first(where: { $0.id == f.miniAppId }),
    ...
    func setViewModeScopedToViewedTracker() {
        let f = freshFixture()
        _ = f.store.setTrackerViewMode(.kanban, miniAppId: f.miniAppId, componentId: f.trackerB)
    ...
    func removeScopedToInactiveComponent() {
        ...
        let checkRow = f.store.addChecklistItem(text: "todo", miniAppId: f.miniAppId)!
        var patch = MiniAppStore.ChecklistItemPatch()
        patch.linkedItems = [ComponentItemRef(componentId: f.trackerA, itemId: f.rowAInA)]
        _ = f.store.patchChecklistItem(id: checkRow, patch: patch, miniAppId: f.miniAppId)

        // Remove rowAInA explicitly via componentId override (active is
        // checklist after addComponent above).
        let ok = f.store.removeItem(id: f.rowAInA, miniAppId: f.miniAppId, componentId: f.trackerA)
        #expect(ok)
        #expect(trackerItem(f.store, miniAppId: f.miniAppId, componentId: f.trackerA, itemId: f.rowAInA) == nil)

        // Cascade should drop the checklist's ref to the now-deleted row.
        guard let miniApp = f.store.miniApps.first(where: { $0.id == f.miniAppId }),
              let comp = miniApp.components.first(where: { $0.id == checkComp }),
...
/// add / toggle / remove, calendar view-mode + add-event). On a miniApp with
...
struct ComponentScopedMutatorOtherKindsTests {
    ...
    private func app() -> (MiniAppStore, UUID) {
        MiniAppTypeRegistry.shared.registerBuiltins()
        let miniApp = MiniApp(name: "T", iconSystemName: "x", typeId: MiniAppType.tracker.id)
        return (MiniAppStore(initial: ([miniApp], miniApp.id)), miniApp.id)
    }

    private func checklist(_ store: MiniAppStore, miniAppId: UUID, componentId: String) -> ChecklistData? {
        guard let m = store.miniApps.first(where: { $0.id == miniAppId }),
              let c = m.components.first(where: { $0.id == componentId }),
              case .checklist(let cl) = c.body else { return nil }
        return cl
    }

    private func calendar(_ store: MiniAppStore, miniAppId: UUID, componentId: String) -> CalendarData? {
        guard let m = store.miniApps.first(where: { $0.id == miniAppId }),
              let c = m.components.first(where: { $0.id == componentId }),
    ...
    func checklistScoped() {
        let (store, miniAppId) = app()
        let clA = store.addComponent(kind: "checklist", name: "A", iconSystemName: "x", miniAppId: miniAppId)!
        let clB = store.addComponent(kind: "checklist", name: "B", iconSystemName: "x", miniAppId: miniAppId)!

        // Add to B (second, viewed). Must not touch A.
        let row = store.addChecklistItem(text: "task", miniAppId: miniAppId, componentId: clB)!
        #expect(checklist(store, miniAppId: miniAppId, componentId: clB)?.items.count == 1)
        #expect(checklist(store, miniAppId: miniAppId, componentId: clA)?.items.isEmpty == true)

        // Toggle done on B's row.
        _ = store.setChecklistItemDone(id: row, done: true, miniAppId: miniAppId, componentId: clB)
        #expect(checklist(store, miniAppId: miniAppId, componentId: clB)?.items.first?.done == true)

        // Remove from B.
        _ = store.removeChecklistItem(id: row, miniAppId: miniAppId, componentId: clB)
        #expect(checklist(store, miniAppId: miniAppId, componentId: clB)?.items.isEmpty == true)
    }

    @Test("calendar view-mode + add-event land on the viewed (non-first) calendar")
    func calendarScoped() {
        let (store, miniAppId) = app()
        let calA = store.addComponent(kind: "calendar", name: "A", iconSystemName: "x", miniAppId: miniAppId)!
        let calB = store.addComponent(kind: "calendar", name: "B", iconSystemName: "x", miniAppId: miniAppId)!

        // Flip B to month view — A stays on its default.
        _ = store.setCalendarViewMode(.month, miniAppId: miniAppId, componentId: calB)
        #expect(calendar(store, miniAppId: miniAppId, componentId: calB)?.viewMode == .month)
        #expect(calendar(store, miniAppId: miniAppId, componentId: calA)?.viewMode != .month)

        // Add an event to B — must not land in A.
        let ev = CalendarEvent(id: UUID(), title: "sync", start: "2026-07-11T10:00:00Z")
        _ = store.addCalendarEvent(ev, miniAppId: miniAppId, componentId: calB)
        #expect(calendar(store, miniAppId: miniAppId, componentId: calB)?.events.count == 1)
        #expect(calendar(store, miniAppId: miniAppId, componentId: calA)?.events.isEmpty == true)
    }
}
