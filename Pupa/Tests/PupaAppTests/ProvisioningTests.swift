...
/// that is the reported "all MiniApps replaced by Daily Briefing" wipe. Instead it
...
struct ProvisioningTests {
    ...
    func freshDeviceWithPopulatedCloudAdoptsAndDoesNotClobber() async throws {
        ...
        let d1 = MiniAppStore()
        let a = d1.addMiniApp(typeId: "tracker", name: "Alpha", iconSystemName: "a.circle")
        let b = d1.addMiniApp(typeId: "tracker", name: "Bravo", iconSystemName: "b.circle")
        let roster = seeded + 2
        #expect(d1.miniApps.count == roster)
        ...
        try await TestStorage.withCloudMirror(cloud) {
            ...
            #expect(!d2.isProvisioning)
            #expect(d2.miniApps.count == roster)                  // adopted, not seeded-over
            #expect(d2.miniApps.contains { $0.id == a })
            #expect(d2.miniApps.contains { $0.id == b })
            #expect(PupaStorage.rosterEstablished)
    ...
    func freshDeviceEmptyCloudSeeds() async throws {
        await MiniAppStore.clearStorage()
        let saved = MiniAppStore.provisioningTimeout
        MiniAppStore.provisioningTimeout = .milliseconds(300)   // don't burn the full wait
        defer { MiniAppStore.provisioningTimeout = saved }
        let cloud = TestStorage.root.appendingPathComponent("cloud-\(UUID().uuidString)", isDirectory: true)
        ...
        try await TestStorage.withCloudMirror(cloud) {
            ...
            #expect(!store.isProvisioning)
            #expect(store.miniApps.count == seeded)               // seeded the defaults
            #expect(PupaStorage.rosterEstablished)
            #expect(FileManager.default.fileExists(
                atPath: PupaStorage.stateRoot.appendingPathComponent("index.json").path))
        }
    ...
    func iCloudOffStillSeedsImmediately() async throws {
        ...
        let store = MiniAppStore()
        #expect(!store.isProvisioning)
        #expect(store.miniApps.count == seeded)
        #expect(PupaStorage.rosterEstablished)
    }
    ...
    func awaitingRosterKeepsGuardArmed() async throws {
        await MiniAppStore.clearStorage()
        let saved = MiniAppStore.provisioningTimeout
        MiniAppStore.provisioningTimeout = .zero          // expire the window immediately
        defer { MiniAppStore.provisioningTimeout = saved }
        ...
        try await TestStorage.withCloudMirror(cloud) {
            ...
            store.renameMiniApp(store.miniApps[0].id, to: "Edited placeholder")
    ...
    func retryAdoptsRosterAndDisarms() async throws {
        ...
        await MiniAppStore.clearStorage()
        let saved = MiniAppStore.provisioningTimeout
        MiniAppStore.provisioningTimeout = .zero
        defer { MiniAppStore.provisioningTimeout = saved }
    ...
    func givenUpPlaceholderNeverPersists() async throws {
        ...
        try await TestStorage.withCloudMirror(cloud) {
            ...
            let placeholder = store.miniApps[0].id
            store.renameMiniApp(placeholder, to: "Edited after giving up")
            ...
            let relaunched = MiniAppStore()
            #expect(!relaunched.miniApps.contains { $0.id == placeholder })
    ...
    func appsCreatedAfterGivingUpPersist() async throws {
        ...
        try await TestStorage.withCloudMirror(cloud) {
            ...
            let placeholder = store.miniApps[0].id
            let mine = store.addMiniApp(typeId: "tracker", name: "Mine", iconSystemName: "m.circle")

            #expect(FileManager.default.fileExists(atPath: bodyURL(mine).path))
            let relaunched = MiniAppStore().miniApps.map(\.id)
    ...
    func adoptingRosterClearsUnsavedFlag() async throws {
        await MiniAppStore.clearStorage()
        let d1 = MiniAppStore()
        _ = d1.addMiniApp(typeId: "tracker", name: "Alpha", iconSystemName: "a.circle")
