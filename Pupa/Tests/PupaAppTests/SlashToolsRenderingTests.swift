...
struct SlashToolsRenderingTests {
    ...
    @Test("Memory scope: Memory + Skill Gates (notifications) + Orchestrator + HITL populated")
    func memoryScopeGroups() {
        ...
        #expect(labels == ["Skill Gates", "Memory", "Orchestrator", "Human-in-the-loop"])

        let memoryNames = groups.first(where: { $0.label == "Memory" })!.tools.map(\.name)
        #expect(Set(memoryNames) == MyAppType.memoryToolNames)
        let gateNames = Set(groups.first(where: { $0.label == "Skill Gates" })!.tools.map(\.name))
        #expect(gateNames == ["get_skill_notifications"])
        let orchNames = groups.first(where: { $0.label == "Orchestrator" })!.tools.map(\.name)
        #expect(Set(orchNames) == MyAppType.orchestratorToolNames)
    }

    @Test("MyApp scope (empty placeholder): Canvas + Skill Gates (memory + notifications), no kind groups")
    func myAppScopeEmptyCanvasGroups() {
        ...
        #expect(groups.map(\.label) == ["Canvas", "Skill Gates", "Human-in-the-loop"])
        let canvasNames = Set(groups.first(where: { $0.label == "Canvas" })!.tools.map(\.name))
        #expect(canvasNames == MyAppType.tracker.baseToolNames)
        let gateNames = Set(groups.first(where: { $0.label == "Skill Gates" })!.tools.map(\.name))
        #expect(gateNames == ["get_skill_memories", "get_skill_notifications"])
    }

    @Test("MyApp scope (all kinds, no skills activated): Canvas + Skill Gates, kind tools hidden")
    func myAppScopeAllKindsGated() {
        ...
        let gateNames = Set(groups.first(where: { $0.label == "Skill Gates" })!.tools.map(\.name))
        #expect(gateNames == [
            "get_skill_tracker", "get_skill_calendar", "get_skill_checklist",
            "get_skill_memories", "get_skill_notifications",
        ])
    }

    @Test("MyApp scope (all skills activated): Canvas + kind groups + Memory + Notifications visible")
    func myAppScopeAllKindsUnlocked() {
        ...
        skillState.activateMemories()
        skillState.activateNotifications()
        let allowed = ChatViewModel.allowedToolNames(scope: .myApp(myApp.id), store: store, skillState: skillState)
        let groups = ChatViewModel.groupFrontendTools(
            descriptors: Self.descriptors(Array(allowed)),
            scope: .myApp(myApp.id),
