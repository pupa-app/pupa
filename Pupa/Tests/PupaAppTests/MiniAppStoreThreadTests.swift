...
/// Tests for the thread-management API on `MiniAppStore`.
///
...
/// in issue #261.
@MainActor
@Suite("MiniAppStore thread management")
struct MiniAppStoreThreadTests {
    ...
    func addThread_appendsAndSelects() {
        ...
        let scope = ChatScope.miniApp(id)
    ...
    func addThread_isolatedToScope() {
        ...
        store.addThread(for: .miniApp(a.id))
    ...
    // MARK: - New MiniApp starts with exactly one thread

    @Test("A newly-created MiniApp has exactly one thread and it is the current thread")
    func newMiniApp_hasOneThread() {
        let (store, id) = makeStore()
        let scope = ChatScope.miniApp(id)
