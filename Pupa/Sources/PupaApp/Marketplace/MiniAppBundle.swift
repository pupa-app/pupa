...
/// A portable, **inert** MiniApp artifact — the marketplace transport unit. It
/// carries only data (a `Codable` `MiniApp` tree + memory files); all rebuild
...
/// the nested `MiniApp` (no parallel relational schema to drift).
public struct MiniAppBundle: Codable, Sendable {
    public let header: Header
    public let app: MiniApp
    public let memories: [MemoryFile]

    public init(header: Header, app: MiniApp, memories: [MemoryFile]) {
        self.header = header
        self.app = app
    ...
    public struct Header: Codable, Sendable {
        ...
        public init(appVersion: String, includedRecords: Bool, includedMemories: Bool, exportedAt: Date = Date()) {
            self.format = MiniAppBundle.formatMagic
            self.formatVersion = MiniAppBundle.currentFormatVersion
    ...
    public static let formatMagic = "pupa.miniapp.bundle"
    public static let legacyFormatMagic = "pupa.myapp.bundle"
