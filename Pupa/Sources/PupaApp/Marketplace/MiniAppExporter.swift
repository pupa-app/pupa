...
/// Builds a `MiniAppBundle` from a live `MiniApp` — the export half of the
...
public enum MiniAppExporter {
    ...
    public static func makeBundle(
        ...
        let header = MiniAppBundle.Header(
            appVersion: appVersion,
            includedRecords: options.includeRecords,
            includedMemories: options.includeMemories)
        return MiniAppBundle(header: header, app: app, memories: memories)
    }

    /// Bundle *every* given app into one `MiniAppLibraryBundle` — the multi-app
    ...
    public static func makeLibraryBundle(
        ...
        let header = MiniAppLibraryBundle.Header(
            appVersion: appVersion,
            appCount: bundles.count,
            includedRecords: includeRecords,
            includedMemories: includeMemories)
        return MiniAppLibraryBundle(header: header, apps: bundles)
    }
