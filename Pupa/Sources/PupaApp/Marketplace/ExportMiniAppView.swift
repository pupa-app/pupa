            ...
            Section {
                NavigationLink {
                    ...
                    SettingsHubRow(icon: "square.and.arrow.up",
                           title: "Share an app",
                           caption: "Send one MiniApp, or all of them, as a .\(MiniAppBundle.fileExtension) bundle")
                }
                NavigationLink {
                    ...
                    SettingsHubRow(icon: "square.and.arrow.down",
                           title: "Import an app",
                           caption: "Load a .\(MiniAppBundle.fileExtension) someone shared, or browse the marketplace")
                }
            } footer: {
                Text("Apps travel as json .\(MiniAppBundle.fileExtension) bundles. Sharing publishes whatever you include. Review the agent prompts on the Share screen before sending.")
            }
        }
...
/// Export a MiniApp to a `.pupa` and hand it to the system share sheet:
...
/// every MiniApp as one library) and **snapshot** (a pin's Export from History /
...
struct ExportShareScreen: View {
    ...
    enum Source {
        ...
        case snapshot(app: MiniApp, meta: SnapshotMeta)
    }
    ...
    @State private var shareDoc: MiniAppDocument?
    ...
    private var snapshotApp: MiniApp? {
    ...
    private var app: MiniApp? {
        if let snapshotApp { return snapshotApp }
        return store.miniApps.first { $0.id == selectedAppId } ?? store.miniApps.first
    }
    ...
    private var content: some View {
        if let app {
            if let meta = snapshotMeta {
                ...
                Section("App") {
                    Picker("App", selection: Binding(
                        ...
                        ForEach(store.miniApps) { Text($0.name).tag($0.id) }
                    }
                }
            }
            ...
            Section {
                ...
                if DeviceInfo.isMac {
                    Text(isAllApps
                         ? "Saves a .\(MiniAppBundle.fileExtension) file with every app. Opening it on another device imports them all into Pupa."
                         : "Saves a .\(MiniAppBundle.fileExtension) file. Opening it on another device imports the app into Pupa.")
                } else {
                    Text(isAllApps
                         ? "Shares a .\(MiniAppBundle.fileExtension) file with every app — AirDrop, Messages, WhatsApp, Mail, or Save to Files. Opening it on another device imports them all into Pupa."
                         : "Shares a .\(MiniAppBundle.fileExtension) file — AirDrop, Messages, WhatsApp, Mail, or Save to Files. Opening it on another device imports the app into Pupa.")
                }
            }
        } else {
    ...
    private func regenerateShareFile() {
        if isAllApps {
            guard !store.miniApps.isEmpty else { clearShareFile(); return }
        ...
        let bundle = MiniAppExporter.makeBundle(
            ...
            options: .init(
                ...
                includeMemories: includeMemories),
            memory: memory)
        writeShareFile(named: MiniAppExporter.exportBaseName(forAppName: app.name)) {
            try bundle.encoded()
        }
    }
    ...
    private func writeShareFile(named base: String, encode: () throws -> Data) {
        ...
        do {
            let data = try encode()
            shareBaseName = base
            shareDoc = MiniAppDocument(data: data)
            if !DeviceInfo.isMac {
        ...
        Form {
            Section {
                Button {
                    ...
                    Label("Choose a .\(MiniAppBundle.fileExtension) file…", systemImage: "folder")
                }
            } footer: {
                Text("Pick a .\(MiniAppBundle.fileExtension) bundle from Files. You can also open one straight from Mail, Messages, or AirDrop.")
            }
            Section {
    ...
    private func handleImport(_ result: Result<URL, Error>) {
        ...
        case .success(let url):
            ...
            do {
                ...
                if MiniAppImporter.probeFormat(data) == .library {
                    let imported = try MiniAppImporter.importLibrary(data, into: store, memory: memory)
                    guard let first = imported.miniAppIds.first else {
                        notice = SharingNotice(title: "Import failed", message: "The bundle had no apps to import.")
                        return
                    }
                    let n = imported.miniAppIds.count
