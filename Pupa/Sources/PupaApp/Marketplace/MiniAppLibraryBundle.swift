...
/// A portable, **inert** bundle of *many* MiniApps — the multi-app transport
/// unit. A thin container over the per-app `MiniAppBundle`: it wraps an array of
/// them plus its own `header`. Same `.pupa` extension as a single-app
/// bundle; the two are told apart by the `header.format` magic (see
/// `MiniAppImporter.probeFormat`). Import loops the per-app authority unchanged,
/// so every per-app guard (settings allow-list, memory path checks,
/// slug-unique rename, caps) applies to each app for free.
public struct MiniAppLibraryBundle: Codable, Sendable {
    public let header: Header
    public let apps: [MiniAppBundle]

    public init(header: Header, apps: [MiniAppBundle]) {
        self.header = header
        self.apps = apps
    }
    ...
    public struct Header: Codable, Sendable {
        ...
        public init(appVersion: String, appCount: Int, includedRecords: Bool,
            ...
            self.format = MiniAppLibraryBundle.formatMagic
            self.formatVersion = MiniAppLibraryBundle.currentFormatVersion
    ...
    public static let fileExtension = MiniAppBundle.fileExtension

    // MARK: - Codec (shares the single-bundle stable JSON codec)

    public func encoded() throws -> Data {
        try MiniAppBundle.makeEncoder().encode(self)
    }
}
