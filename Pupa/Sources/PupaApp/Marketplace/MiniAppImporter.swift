...
/// Rebuilds a `MiniApp` from a `MiniAppBundle` — the import half of the
...
public enum MiniAppImporter {
    ...
    static let allowedSettingKeys: Set<String> = [
        MiniAppStore.llmProviderSettingsKey,
        MiniAppStore.llmModelSettingsKey,
    ]
    ...
    public static func probeFormat(_ data: Data) -> BundleFormat {
        ...
        guard data.count <= maxLibraryBytes,
              let probe = try? MiniAppBundle.makeDecoder().decode(Probe.self, from: data) else {
            return .unknown
        }
        switch probe.header.format {
        case MiniAppBundle.formatMagic, MiniAppBundle.legacyFormatMagic: return .single
        case MiniAppLibraryBundle.formatMagic: return .library
    ...
    public static func importBundle(
        ...
        do {
            bundle = try MiniAppBundle.makeDecoder().decode(MiniAppBundle.self, from: data)
        } catch {
    ...
    static func importDecoded(
        ...
        guard bundle.header.format == MiniAppBundle.formatMagic
                || bundle.header.format == MiniAppBundle.legacyFormatMagic else {
            throw ImportError.notABundle
        }
        guard bundle.header.formatVersion <= MiniAppBundle.currentFormatVersion else {
            throw ImportError.newerFormat(found: bundle.header.formatVersion,
                                          supported: MiniAppBundle.currentFormatVersion)
        }
        ...
        guard let type = MiniAppTypeRegistry.shared.resolve(id: decoded.typeId) else {
        ...
        cleanSettings[MiniAppStore.remoteImagesSettingsKey] = .bool(false)
        ...
        // Stage 4 — insert.
        let id = store.importMiniApp(app)
        ...
        return ImportResult(miniAppId: id, warnings: warnings)
    }

    /// Import a multi-app `MiniAppLibraryBundle`. **Best-effort:** each app runs
    ...
    public static func importLibrary(
        ...
        do {
            library = try MiniAppBundle.makeDecoder().decode(MiniAppLibraryBundle.self, from: data)
        } catch {
            throw ImportError.notABundle
        }
        guard library.header.format == MiniAppLibraryBundle.formatMagic else { throw ImportError.notABundle }
        guard library.header.formatVersion <= MiniAppLibraryBundle.currentFormatVersion else {
            throw ImportError.newerFormat(found: library.header.formatVersion,
                                          supported: MiniAppLibraryBundle.currentFormatVersion)
        }
        ...
        for appBundle in library.apps {
            do {
                ...
                ids.append(result.miniAppId)
                warnings.append(contentsOf: result.warnings)
            } catch {
                warnings.append("Skipped '\(appBundle.app.name)': \(error.localizedDescription)")
            }
        }
        return LibraryImportResult(miniAppIds: ids, warnings: warnings)
    }
    ...
    private static func sanitizeSettings(
        ...
        if case .string(let provider)? = clean[MiniAppStore.llmProviderSettingsKey],
           case .string(let model)? = clean[MiniAppStore.llmModelSettingsKey],
           !provider.isEmpty, !model.isEmpty {
            // Plausible pair — keep.
        } else {
            clean[MiniAppStore.llmProviderSettingsKey] = nil
            clean[MiniAppStore.llmModelSettingsKey] = nil
        }
    ...
    private static func uniqueName(base: String, store: MiniAppStore) -> String {
