...
/// top-level agent dropdown (it belongs to no MyApp). Below the roster,
/// two settings sections hold the A2A guardrails (`AgentInvocationGate`:
/// conversation rounds per agent pair + max chain depth) and the
/// per-turn tool-round limit. A final list of conversation threads is
/// grouped by agent, one collapsible group each.
///
