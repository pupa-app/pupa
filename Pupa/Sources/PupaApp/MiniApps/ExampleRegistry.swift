...
/// Common interface every seeded example MiniApp must satisfy.
///
/// Each conforming type is responsible for building its `MiniApp` value and
...
public protocol ExampleMiniApp {
    /// Display name. Also the idempotency key used by `MiniAppStore` to
    ...
    /// Build a fully-populated `MiniApp` ready to append to the store.
...
public enum ExampleRegistry {
    ...
    public nonisolated(unsafe) static let all: [any ExampleMiniApp.Type] = [
    ...
    /// first is the active one. Two, not one: with a single app the MiniApps
    ...
    public static func seedAgentsMd(
        ...
        example(named: name)?.seedAgentsMd(
            globalMemory: globalMemory, appRoot: MemoryStore.appRoot(miniAppId: id))
    }

    /// Look up an example type by its `name`. Returns `nil` when not found.
    public static func example(named name: String) -> (any ExampleMiniApp.Type)? {
