...
/// Seeded "Job Search & Apply" MiniApp — the published marketplace app, shipped
/// in-app.
///
...
///
/// Seeded — not imported — so it keeps the normal MiniApp defaults (remote
/// images allowed); `MiniAppImporter`'s untrusted-bundle hardening applies to
/// what a user downloads, not to what we ship.
enum JobSearchExample: ExampleMiniApp {
    /// Display name used both as the seed's `MiniApp.name` and as the
    /// idempotency key for `restoreExampleMiniApp()` (a MiniApp with this
    ...
    /// Build a fresh `MiniApp` from the bundle: its components verbatim, a fresh
    /// id, thread and `createdAt` so a restored copy never collides with a
    /// hand-edited one.
    static func make() -> MiniApp {
        let app = bundle.app
    ...
    /// Settings → "Restore example MiniApp" tap.
    ///
    ...
    static var resourceURL: URL? {
        Bundle.module.url(forResource: resourceName, withExtension: MiniAppBundle.fileExtension)
    }
    ...
    /// both.
    static let bundle: MiniAppBundle = {
        guard let url = resourceURL,
              let data = try? Data(contentsOf: url),
              let decoded = try? MiniAppBundle.makeDecoder().decode(MiniAppBundle.self, from: data)
        else {
            preconditionFailure("Missing or malformed \(resourceName).\(MiniAppBundle.fileExtension) resource")
        }
        return decoded
    }()
}
