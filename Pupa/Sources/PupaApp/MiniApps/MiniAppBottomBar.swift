...
/// Persistent bottom bar for a miniApp or the orchestrator — the per-subject
/// "tab bar". Always visible (mounted via `.safeAreaInset`) on a miniApp's home /
...
/// own look.
public struct MiniAppBottomBar: View {
    ...
    let subject: MiniAppHomeView.Subject
    ...
    /// Open the Change History sheet for a miniApp. Only wired for `.miniApp`; the
    ...
    /// Present the MiniApps sheet. The bar is the only way there now that the
    ...
    public init(
        subject: MiniAppHomeView.Subject,
        currentPage: Page,
        appColor: Color,
        ...
        self.onOpenMiniApps = onOpenMiniApps
    }
    ...
    private var homeSelection: SidebarSelection {
        miniAppId.map { .miniAppHome($0) } ?? .orchestrator
    }

    private var memoriesSelection: SidebarSelection {
        miniAppId.map { .miniAppMemories($0) } ?? .orchestratorMemories
    }

    private var agentsSelection: SidebarSelection {
        miniAppId.map { .miniAppAgents($0) } ?? .orchestratorAgentDetail
    }
    ...
    private func perform(_ row: BarMenuRow) {
        ...
        case .history: if let id = miniAppId { onShowHistory(id) }
        case .orchestrator: onSelect(.orchestrator)
        case .miniApps: onOpenMiniApps?()
    ...
    private var menuRows: [BarMenuRow] {
        BarMenuRow.rows(isMiniApp: miniAppId != nil, hasMiniApps: onOpenMiniApps != nil)
    }

    /// The app's menu, grouped one axis per section: this scope's pages
    /// (Agents, History), which scope you're in (MiniApps, Orchestrator), then
    /// app-wide (Settings). Mixing those in one flat list put History next to
    /// Settings, two rows that don't even apply to the same thing; MiniApps is a
