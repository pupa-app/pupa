...
/// One row in the sidebar. Each miniApp owns its own chat thread list and a list
/// of components (a tracker, a calendar, …) displayed under it in the sidebar.
/// MiniApps of the same `typeId` share a tool surface and a system-prompt
/// fragment (resolved via `MiniAppTypeRegistry`).
public struct MiniApp: Codable, Hashable, Identifiable, Sendable {
    public let id: UUID
    public var name: String
    public var iconSystemName: String
    public var typeId: String
    /// Ordered list of components rendered under this MiniApp. Each component
    /// has a stable string id (e.g. `"tracker-1"`, `"calendar-1"`) the agent
    /// can address. New MiniApps start with a single component matching
    ...
    /// Ordered list of chat threads for this miniApp. Always non-empty.
    ...
    public var allowsRemoteImages: Bool {
        if case .bool(let allowed)? = settings[MiniAppStore.remoteImagesSettingsKey] {
            return allowed
        }
        return true
    }

    /// Per-MiniApp settings overrides. Keys are `SettingsKey.name` values.
    ...
    /// `MiniAppStore` backfills those from creation order on load. User-settable
