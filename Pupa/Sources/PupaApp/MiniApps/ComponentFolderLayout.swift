...
/// UI-only folder grouping for a MiniApp's component tiles. Presentational —
...
/// keyed by MiniApp `id.uuidString`. Not part of per-app History snapshots.
...
/// Per-MiniApp layout: the folders and which component each lives in.
/// `assignments` maps componentId → folderId; unlisted components are loose.
/// Component ids are unique within one MiniApp, so they key `assignments`.
