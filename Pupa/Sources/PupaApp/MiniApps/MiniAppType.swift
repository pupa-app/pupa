...
/// Everything a `MiniAppType` declares about one component kind, in one place:
...
/// A registered template for a kind of miniApp. Declares the agent's surface
/// area for the type and slices it by component kind so a MiniApp instance only
...
/// `MiniApp.components` on every agent round, so the surface grows mid-turn
/// the instant a component is added and shrinks the instant the last one of
/// a kind is removed.
public struct MiniAppType: Sendable, Hashable, Identifiable {
    ...
    /// Tools always advertised to this MiniApp's agent — component lifecycle
    ...
    /// Component kinds this MiniAppType may host — derived from `kinds`. Used by
    ...
    /// Resolve the tools advertised to a MiniApp's agent given the kinds of
    ...
    /// .miniApp scope (the memories surface is not miniApp-scoped, but per-miniApp
    ...
    /// this MiniApp. Always advertised at MiniApp scope — to the main agent AND to
    ...
    /// either a miniApp agent or the orchestrator can post banners.
    ...
    /// memory-mode agent see and create miniApps and delegate a one-shot
    /// prompt to any existing miniApp's agent — sub-runs use a fresh
    /// `threadId` against the target miniApp's normal tool surface (canvas
    /// mutators + memories), so they can mutate that miniApp's canvas the same
    /// way the user's own chat would. Not exposed inside any miniApp's own
    /// scope; only registered on the `.memory` session.
    public static let orchestratorToolNames: Set<String> = Set([
        "listMiniApps",
        "createMiniApp",
        "renameMiniApp",
        "setMiniAppIcon",
        "setMiniAppColor",
        "invokeMiniAppAgent",
    ]).union(legacyOrchestratorToolAliases.keys)

    public static let legacyOrchestratorToolAliases: [String: String] = [
        "listMyApps": "listMiniApps",
        "createMyApp": "createMiniApp",
        "renameMyApp": "renameMiniApp",
        "setMyAppIcon": "setMiniAppIcon",
        "setMyAppColor": "setMiniAppColor",
        "invokeMyAppAgent": "invokeMiniAppAgent",
    ]

    public static func canonicalToolName(_ name: String) -> String {
        legacyOrchestratorToolAliases[name] ?? name
    }

    public static func expandedDisabledToolNames(_ names: Set<String>) -> Set<String> {
        var expanded = names
        for (legacy, current) in legacyOrchestratorToolAliases
        where names.contains(legacy) || names.contains(current) {
            expanded.insert(legacy)
            expanded.insert(current)
        }
        return expanded
    }

    /// The built-in tracker miniApp type. Drives the existing tracker UI and
    /// also hosts calendar components — a single MiniApp can mix kinds.
    public static let tracker = MiniAppType(
        id: "tracker",
        displayName: "Tracker",
        ...
        `componentId` to target one; single-component miniApps may omit it.
