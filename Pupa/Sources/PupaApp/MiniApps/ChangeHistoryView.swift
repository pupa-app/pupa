...
/// given MiniApp, grouped by calendar day. The newest snapshot is the current
...
public struct ChangeHistoryView: View {
    ...
    private var app: MiniApp? { store.miniApps.first { $0.id == miniAppId } }
    private var appColor: Color { .color(atIndex: store.colorIndex(for: miniAppId)) }

    /// Both hit storage or scan the event log, so `body` reads each exactly
    /// once into a local and passes it down — never per row.
    private var snapshots: [SnapshotMeta] { store.snapshots(forMiniApp: miniAppId) }
    private var events: [ItemEvent] { store.itemEventLog.events(forMiniApp: miniAppId) }
    ...
    public var body: some View {
        ...
        Group {
            if snaps.isEmpty {
                ContentUnavailableView(
                    ...
                    description: Text("As you and the agent edit this MiniApp, restore points appear here.")
                )
            } else {
                List {
        ...
        .background {
            SnapshotLabelAlert(isPresented: $showingSnapshotPrompt) { label in
                store.takeSnapshot(miniAppId: miniAppId, label: label)
            }
        }
        ...
        .confirmationDialog(
            ...
            Button("Restore") {
                if let snap = pendingRestore { store.restore(miniAppId: miniAppId, snapshotId: snap.id) }
                pendingRestore = nil
            }
        ...
        // Pinned page header so it's always clear this is a MiniApp's History —
        // matches the eyebrow + name header on the other MiniApp pages.
    ...
    private func exportDestination(_ snap: SnapshotMeta) -> some View {
        if let memory = store.globalMemory,
           let resolved = store.restoredApp(forSnapshot: snap.id, appId: miniAppId) {
            ExportShareScreen(
