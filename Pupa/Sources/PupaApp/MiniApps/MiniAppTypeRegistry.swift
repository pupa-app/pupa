...
/// Append-only registry of `MiniAppType`s the app knows how to render. The
...
@MainActor
public final class MiniAppTypeRegistry {
    public static let shared = MiniAppTypeRegistry()

    private var types: [MiniAppType] = []

    private init() {}

    /// Register a type. Re-registering an id replaces the existing one.
    public func register(_ type: MiniAppType) {
    ...
    public func resolve(id: String) -> MiniAppType? {
        types.first { $0.id == id }
    }

    public var allTypes: [MiniAppType] { types }
    ...
    public func registerBuiltins() {
        if resolve(id: MiniAppType.tracker.id) == nil {
            register(.tracker)
        }
