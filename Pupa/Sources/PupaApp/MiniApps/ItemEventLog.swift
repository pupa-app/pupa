...
public enum ItemEventKind: String, Codable, Sendable {
    ...
    case linked
    case unlinked
    /// The MiniApp was reverted to an earlier snapshot (see `SnapshotStore`).
    case restored
...
/// One entry in a MiniApp's append-only change feed. Captions the History
/// timeline (see `MiniAppStore.changeSummary`); the restore unit is the
...
public struct ItemEvent: Codable, Sendable, Identifiable {
    ...
    enum CodingKeys: String, CodingKey {
        case id, timestamp, miniAppId, myAppId, componentId, kind, actor, itemId, threadId
    }
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.miniAppId = try c.decodeIfPresent(UUID.self, forKey: .miniAppId)
            ?? c.decode(UUID.self, forKey: .myAppId)
...
public struct ItemEventLog: Codable, Sendable {
    ...
    public func events(forMiniApp miniAppId: UUID) -> [ItemEvent] {
        events.filter { $0.miniAppId == miniAppId }
    }
