...
/// creation time and stored in `MiniApp.threads` / `MiniAppStore.memoryThreads`.
/// The active one is tracked by `MiniApp.currentThreadId` /
/// `MiniAppStore.memoryCurrentThreadId`.
public struct ChatThread: Codable, Hashable, Identifiable, Sendable {
    ...
    /// the thread inherits its scope's default (MiniApp / orchestrator).
