...
/// Detail-pane landing page shown when a MiniApp is opened. For a miniApp it is
...
public struct MiniAppHomeView: View {
    ...
    /// page, so it opens expanded. A miniApp's Home is its component grid; the
    ...
    private var miniAppId: UUID? {
        if case .miniApp(let id) = subject { return id }
        return nil
    }

    private var miniApp: MiniApp? {
        guard let id = miniAppId else { return nil }
        return store.miniApps.first(where: { $0.id == id })
    }

    private var appColor: Color {
        guard let id = miniAppId else { return .orchestratorColor }
    ...
    public var body: some View {
        ...
        .sheet(item: $editingComponent) { ref in
            if let component = store.miniApps
                .first(where: { $0.id == ref.miniAppId })?
        ...
        .alert(
            ...
            Button("Delete", role: .destructive) {
                store.removeComponent(componentId: ref.componentId, miniAppId: ref.miniAppId)
                deletingComponent = nil
            }
            Button("Cancel", role: .cancel) { deletingComponent = nil }
        } message: { ref in
            if let name = store.miniApps
                .first(where: { $0.id == ref.miniAppId })?
        ...
        .alert(
            ...
            Button("Rename") {
                ...
                if !trimmed.isEmpty {
                    store.renameComponentFolder(folderId: ref.folderId, name: trimmed, miniAppId: ref.miniAppId)
                }
                renamingFolder = nil
            }
    ...
    private func header(
        ...
        HStack(alignment: .top, spacing: 12) {
            MiniAppPageHeader(page: "Home", name: name, icon: icon, color: appColor)
            trailing()
        }
    }
    ...
    /// Folder layout is UI-only (`MiniAppStore.componentFolders`) — invisible to
    /// the agent and to marketplace exports.
    private func componentsPanel(_ app: MiniApp) -> some View {
        let layout = store.componentFolderLayout(forMiniApp: app.id)
                ...
                ForEach(layout.folders) { folder in
                    FolderGridTile(
                        ...
                        onRename: {
                            ...
                            renamingFolder = FolderRef(miniAppId: app.id, folderId: folder.id)
                        },
                        onDelete: { store.removeComponentFolder(folderId: folder.id, miniAppId: app.id) },
                        onDropComponent: { dragged in
                            store.setComponentFolder(componentId: dragged, folderId: folder.id, miniAppId: app.id)
                        }
                    )
                }
        ...
        .dropDestination(for: String.self) { ids, _ in
            ...
            store.setComponentFolder(componentId: dragged, folderId: nil, miniAppId: app.id)
            return true
        }
    }

    /// MiniApp-level lock: toggles the lock on every component at once. Shows
    /// locked only when all components are locked. Hidden when the MiniApp has
    /// no components to lock.
    @ViewBuilder
    private func allComponentsLockButton(_ app: MiniApp) -> some View {
        if !app.components.isEmpty {
            let allLocked = store.areAllComponentsLocked(miniAppId: app.id)
            Button {
                store.setAllComponentsLocked(locked: !allLocked, miniAppId: app.id)
            } label: {
    ...
    private func gridTile(app: MiniApp, component: Component, inFolder: Bool) -> some View {
        ComponentGridTile(
            component: component,
            appColor: appColor,
            ...
            inFolder: inFolder,
            onTap: { onNavigate(.miniAppComponent(app.id, component.id)) },
            onEditMeta: { editingComponent = EditingComponentRef(miniAppId: app.id, componentId: component.id) },
            onDelete: { deletingComponent = EditingComponentRef(miniAppId: app.id, componentId: component.id) },
            onMoveOut: { store.setComponentFolder(componentId: component.id, folderId: nil, miniAppId: app.id) },
            onCombine: { dragged in
                store.combineComponentsIntoFolder(dragged, component.id, miniAppId: app.id)
            }
        )
    ...
    private func members(of folder: ComponentFolder, in app: MiniApp,
    ...
    private func expandedFolderSection(app: MiniApp, folder: ComponentFolder,
    ...
    /// the component (`MiniAppStore.addComponent`) and opens it; name/icon can be
    /// tweaked afterward from the tile's context menu.
    private func addComponentMenu(_ app: MiniApp) -> some View {
        Menu {
            ForEach(availableKinds, id: \.self) { kind in
                Button {
                    if let newId = store.addComponent(
                        ...
                        onNavigate(.miniAppComponent(app.id, newId))
                    }
                } label: {
    ...
    /// what the app type supports. Mirrors `NewMiniAppSheet`.
    private var availableKinds: [String] {
        let appType = MiniAppTypeRegistry.shared.allTypes.first ?? .tracker
    ...
    /// Orchestrator Outline: why it's special + the miniapps it can drive (each
    /// row jumps to that miniapp's home). Leads the page (expanded by default).
    private func orchestratorOutlinePanel() -> some View {
        DisclosureGroup(isExpanded: $outlineExpanded) {
            VStack(alignment: .leading, spacing: 12) {
                Text("The orchestrator is a global agent with its own chat and shared memory that spans every miniapp. Use it to plan, delegate work to a miniapp's agent, or spin up a new miniapp, without opening any single canvas.")
                    .font(.callout)
                    .foregroundStyle(.secondary)

                if !store.visibleMiniApps.isEmpty {
                    Divider()
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ForEach(store.visibleMiniApps) { app in
                            orchestrableRow(app)
                        }
                    }
                }
    ...
    private func orchestrableRow(_ app: MiniApp) -> some View {
        Button {
            onNavigate(.miniAppHome(app.id))
        } label: {
            HStack(spacing: 10) {
...
/// recursive row and its `MiniAppMemoriesView` host share one closure set. Folder
...
/// Recursive memory row used by the `MiniAppMemoriesView` browse page (reached
...
struct MemoryLandingRow: View {
    ...
    /// Maps a tapped file's path to the selection to open — `.miniAppMemoryFile`
    /// for a miniapp, `.memoryFile` for the orchestrator — so one row serves
    /// both scopes.
...
/// ids (`"tracker-1"`) repeat across miniApps, so the miniApp id is part of the
/// identity.
private struct EditingComponentRef: Identifiable {
    let miniAppId: UUID
    let componentId: String
    var id: String { "\(miniAppId.uuidString)/\(componentId)" }
}

/// Identifies the folder being renamed. Folder ids are unique within a MiniApp.
private struct FolderRef: Identifiable {
    let miniAppId: UUID
    let folderId: String
    var id: String { "\(miniAppId.uuidString)/\(folderId)" }
}
...
private struct FolderGridTile: View {
    ...
    /// Folders are theme-neutral (gray), not tinted with the MiniApp color, so
